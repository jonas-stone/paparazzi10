...
clf = joblib.load('../ground_detector.pkl')
print('Model loaded.')
...
SAVE_PATH   = '../ground_detection_results.png'  # filename to save the figure to
...
# Each image gets 2 columns (original | detection), rows = NUM_IMAGES / 2
cols_per_image = 2
grid_cols = int(np.sqrt(NUM_IMAGES)) * cols_per_image   # e.g. 16 images → 8 columns
grid_rows = int(np.sqrt(NUM_IMAGES))                    # e.g. 16 images → 4 rows
