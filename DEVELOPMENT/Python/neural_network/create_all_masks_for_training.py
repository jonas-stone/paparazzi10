...
# Test mode
TEST_MODE       = False  # leave to false unless you test stuff
TEST_NUM_IMAGES = 30
...
def extract_features(yuv, hsv, lab, y, x):
    """
    same as  all other feature extractors used for the NN
    """
    p_yuv = yuv[y, x]
    p_hsv = hsv[y, x]
    p_lab = lab[y, x]
...
def run_classifier(clf, img):
    """
    Keeps uint8 inputs so mean and standard deviation match the training which is float64 exactly.
    """
    h, w, _ = img.shape
...
def filter_tree_by_edge_density(raw_mask, img):
    """
    tree detection script logic basically
    """
...
def make_tree_mask(tree_clf, img):
    """
    applis the whole thing to check for edges
    """
...
def process_image(args):
    ...
    ...
    """
    Loads one imags and generates all three masks and saves them immediately.
    """
