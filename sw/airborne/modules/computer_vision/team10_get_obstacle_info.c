...
/*
    ...
    */
    ...
    // ── SAFE COLUMN BAR ───────────────────────────────────────────────────
    // Split image height (= viewer width) into 7 columns
    // Green = no obstacle in that column, black = obstacle present
    {
        uint8_t *src = (uint8_t *)img->buf;
        int col_width = h / 7;
        ...
        for (int col = 0; col < 7; col++) {
            ...
            // Check if any obstacle overlaps this column
            int has_obstacle = 0;
            for (int i = 0; i < no; i++) {
                int s = (int)oo[i].start;
                int e = s + (int)oo[i].width - 1;
                // Convert to viewer coords (same flip as detection bar)
                int vs = h - 1 - e;
                int ve = h - 1 - s;
                if (vs < 0) vs = 0;
                if (ve >= h) ve = h - 1;
                // Check overlap with this column's y range
            ...
            // Paint the last 2 columns of buffer for this bar
            // (just above the detection bar which is at w-12 to w-2)
