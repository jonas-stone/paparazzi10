/*
 * team10_get_obstacle_info_standalone_debug.c
 *
 * IDENTICAL pipeline to the original, but with added debug snapshot
 * buffers so that the Python visualizer can retrieve intermediate
 * masks after each processing step for report figures.
 *
 * Added:
 *   - debug_mask_after_green[]    snapshot of work_mask after detect_green_ground_ml
 *   - debug_mask_after_blob[]     snapshot of work_mask after isolate_ground_blob
 *   - debug_mask_after_holes[]    snapshot of work_mask after fill_holes_mask
 *   - debug_mask_flipped[]        snapshot of the horizontally flipped mask
 *   - debug_plant_green_lax[]     snapshot of plant_mask_small (lax green, before subtract)
 *   - debug_plant_after_sub[]     snapshot of plant_mask_small after ground subtraction
 *   - get_debug_*() getter functions exposed to Python via ctypes
 *   - get_work_mask() / get_plant_mask() from the original wrapper
 */
...
static int16_t  cc_bb_x0[MAX_CC_LABELS];
...
/* ══════════════════════════════════════════════════════════════════════════════
 *  DEBUG SNAPSHOT BUFFERS (new)
 * ══════════════════════════════════════════════════════════════════════════════ */
static uint8_t  debug_mask_after_green[MAX_PIXELS];   /* after detect_green_ground_ml  */
static uint8_t  debug_mask_after_blob[MAX_PIXELS];    /* after isolate_ground_blob     */
static uint8_t  debug_mask_after_holes[MAX_PIXELS];   /* after fill_holes_mask         */
static uint8_t  debug_mask_flipped_copy[MAX_PIXELS];  /* the horizontally flipped mask */
static uint8_t  debug_plant_green_lax[MAX_PLANT_PIXELS]; /* lax green before subtract  */
static uint8_t  debug_plant_after_sub[MAX_PLANT_PIXELS]; /* plant mask after subtract  */

static int      debug_w = 0, debug_h = 0;             /* image dimensions for getters  */
static int      debug_pw = 0, debug_ph = 0;            /* plant mask dimensions         */

/* ══════════════════════════════════════════════════════════════════════════════
 *  GETTER FUNCTIONS (exposed to Python)
 * ══════════════════════════════════════════════════════════════════════════════ */
uint8_t *get_work_mask(void)              { return work_mask; }
uint8_t *get_plant_mask(void)             { return plant_mask_full; }
uint8_t *get_debug_after_green(void)      { return debug_mask_after_green; }
uint8_t *get_debug_after_blob(void)       { return debug_mask_after_blob; }
uint8_t *get_debug_after_holes(void)      { return debug_mask_after_holes; }
uint8_t *get_debug_flipped(void)          { return debug_mask_flipped_copy; }
uint8_t *get_debug_plant_green_lax(void)  { return debug_plant_green_lax; }
uint8_t *get_debug_plant_after_sub(void)  { return debug_plant_after_sub; }
int      get_debug_w(void)                { return debug_w; }
int      get_debug_h(void)                { return debug_h; }
int      get_debug_pw(void)               { return debug_pw; }
int      get_debug_ph(void)               { return debug_ph; }
...
/* ══════════════════════════════════════════════════════════════════════════════
 *  MAIN PIPELINE (with debug snapshots)
 * ══════════════════════════════════════════════════════════════════════════════ */
                          ...
                          int bro[], int *gfo, float *gfro)
{
    ...
    /* ── DEBUG SNAPSHOT: after green masking ── */
    memcpy(debug_mask_after_green, work_mask, w * h);

    uint8_t no = 0;
    if (ground) {
        isolate_ground_blob(work_mask, w, h, DEFAULT_BLOB_AREA_THRESH);

        /* ── DEBUG SNAPSHOT: after blob isolation ── */
        memcpy(debug_mask_after_blob, work_mask, w * h);

        fill_holes_mask(work_mask, w, h);

        /* ── DEBUG SNAPSHOT: after hole filling ── */
        memcpy(debug_mask_after_holes, work_mask, w * h);

        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++) work_flipped[y*w+x] = work_mask[y*w+(w-1-x)];

        /* ── DEBUG SNAPSHOT: flipped mask ── */
        memcpy(debug_mask_flipped_copy, work_flipped, w * h);
        ...
        /* no ground: clear debug buffers */
        memset(debug_mask_after_blob, 0, w * h);
        memset(debug_mask_after_holes, 0, w * h);
        memset(debug_mask_flipped_copy, 0, w * h);
    }
    ...
    if (po != NULL) {
        ...
        /* ── DEBUG SNAPSHOT: lax green (before ground subtraction) ── */
        debug_pw = pw;
        debug_ph = ph;
        memcpy(debug_plant_green_lax, plant_mask_small, pw * ph);
        ...
        /* ── DEBUG SNAPSHOT: plant mask after ground subtraction ── */
        memcpy(debug_plant_after_sub, plant_mask_small, pw * ph);
