...
Note there is an option to meger labels for the
"""
...
# Settings and stuff
"""
Label mode: '3strip' (3 strip labels (1=left, 2=centre, 3=right, 4=turn)), '5strip' (5 strip labels (1-5=strips, 
6= turn)), '5merged' (5 strip JSON but merged to 4 classes (1+2→1, 3→2, 4+5→3, 6→4))

The labels are stored in a JSON file which is made using another software. They are stored in json for eeasue of use.
The logic behind the labeling is the one in the mask_viewer.py file. You can try various configuration if you want, not
just 3 strip but strip is the most successful

note the paths are absolute and need to be changed to work on the relevant computers

Also there is an option to create turn the 5 strip labels into 3 strip ones by merging the 2 left strips and the 2 right
ones. This approach is wrong and has been left for the sake of completeness. The most successful apporach is the standard
3 strip one
...
LABELS_JSON_3STRIP = os.path.join(SCRIPT_DIR, 'labels_3strip.json')
LABELS_JSON_5STRIP = os.path.join(SCRIPT_DIR, 'labels.json')
OUTPUT_DIR = os.path.join(SCRIPT_DIR, 'model')


IMAGES_DIR = os.path.join(SCRIPT_DIR, '..', '..', 'downloads from drone', '20260320')
...
BATCH_SIZE    = 32
LEARNING_RATE = 0.003
LR_DECAY      = 1.0     # no decay used because it showed worse results at various values
L2_REG        = 2e-3
RANDOM_SEED   = 42
PATIENCE      = 60

# Class balancing
MAX_SAMPLES_PER_CLASS = None # Cap the number of samples per class before training, found to not help
AUGMENT = True # data augmentation switch
AUG_FLIP = True # data augmentation using the flip of the first and last strips
AUG_BRIGHTNESS = True # data augmentation using brightness
AUG_NOISE = True # data augmentation using noise
AUG_NOISE_STD = 0.02 # data augmentation using noise standard diviation
AUG_BRIGHTNESS_RANGE = 0.25

# Settings for the various labels
...
def main():
    ...
    for i, e in enumerate(entries):
        ...
        vec = load_and_preprocess(os.path.join(IMAGES_DIR, os.path.basename(e['path'])))
    ...
    # Label merging  (if applicable and unused because this is a super wrong approach as it turned out)
    if cfg['merge_map']:
    ...
    if AUGMENT:
        ...
        X_trn, y_trn = build_augmented_dataset(X_trn, y_trn, rng, cfg['flip_map'], cfg['flip_skip'])
        shuf = rng.permutation(len(X_trn))
        X_trn, y_trn = X_trn[shuf], y_trn[shuf]
        print(f'{len(X_trn)} samples)')

    # Re maping to indices
    ...
    print(f'\nAfter the augmentation: train={len(X_trn)}  val={len(X_val)}  test={len(X_tst)}')

    # Initialising weights with the He approach
    ...
    # Train
    print(f'\nTraining  epochs={EPOCHS}  lr={LEARNING_RATE}->decay*{LR_DECAY}  l2={L2_REG}  patience={PATIENCE}')
    print('-' * 60)
    ...
    for epoch in range(1, EPOCHS+1):
        lr *= LR_DECAY # decay learning rate each epoch
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            t  = time.time()-t1
            is_best = va > best_val
            print(f'  epoch {epoch:4d}/{EPOCHS}  train ={ta:5.1f}%  val={va:5.1f}%  lr={lr:.5f}  t={t:.1f}s'
